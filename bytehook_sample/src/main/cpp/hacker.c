...
//        bytehook_dump_records(fd, BYTEHOOK_RECORD_ITEM_CALLER_LIB_NAME | BYTEHOOK_RECORD_ITEM_OP | BYTEHOOK_RECORD_ITEM_LIB_NAME | BYTEHOOK_RECORD_ITEM_SYM_NAME | BYTEHOOK_RECORD_ITEM_ERRNO | BYTEHOOK_RECORD_ITEM_STUB);
        close(fd);
    }
...
static void hacker_jni_do_run(JNIEnv *env, jobject thiz, jint benchmark)
{
  ...
  if (NULL != sample_test_strlen)
    sample_test_strlen(benchmark);
}
...
static JNINativeMethod hacker_jni_methods[] = {
    {
        "nativeBytehookHook",
        "()I",
        (void *) hacker_jni_bytehook_hook
    },
    {
        "nativeBytehookUnhook",
        "()I",
        (void *) hacker_jni_bytehook_unhook
    },
    {
        "nativeDumpRecords",
        "(Ljava/lang/String;)V",
        (void *)hacker_jni_dump_records
    },
    {
        "nativeDoDlopen",
        "()V",
        (void *)hacker_jni_do_dlopen
    },
    {
        "nativeDoDlclose",
        "()V",
        (void *)hacker_jni_do_dlclose
    },
    {
        "nativeDoRun",
        "(I)V",
        (void *)hacker_jni_do_run
    }
};
                                  ...
                                  void *reserved)
{
    ...
    if(0 != (*env)->RegisterNatives(env, cls, hacker_jni_methods, sizeof(hacker_jni_methods) / sizeof(hacker_jni_methods[0]))) return JNI_ERR;
